...
import { getCategorieById, getConpanyInformations, getCustomerById, getProductsByCategorie} from "@/app/crud";
...
const OrdersPage = (props: propsParams) => {
    ...
    const [categorie, setCategorie] = useState<CategorieParams | undefined>(undefined)
  ...
  useEffect(() => {
    const getProduct = async() => {
        const categorieId = (await props.params).categorieId;
        const categorie = await getCategorieById(categorieId)
        const products = await getProductsByCategorie(categorieId);
                        ...
                        <CustomerDataContext.Provider value={customerData}>
                            <BannerContext.Provider value={{bannerexist: bannerForEveryThing, bannerText: bannerText, bannerStatus: bannerStatus , setBanner: setBanner}}>
                              ...
                              <div style={style} className='page'>
                                { categorie && <ProductsShowing categorie={categorie} products={products?? []}/> }
                              </div>
                                <About/>
